...
export function addFriend(name) {
  return {type: types.ADD_FRIEND, name};
}

export function deleteFriend(id) {
  return {type: types.DELETE_FRIEND, id};
}

export function starFriend(id) {
  return {type: types.STAR_FRIEND, id};
}

function fetchList(books) {
  return {type: types.BOOK_LIST, books}
}
  ...
  return function(dispatch, getState) {
    return fetch(API_ROOT + 'users/register', {
      ...
      .then(data => {
        ...
        if (data.status === 0) {
          dispatch({type: types.REGISTER, username: data.username});
        } else {
          alert(data.err_msg);
        }
      });
